...
for modIndex in range(len(baseFile)):
  ...
  for btwn in range(0, 256):
    ...
    try:
      with open(f"{baseDir}/binary/{fileName}.bin", 'wb') as out, open(f"{baseDir}/binary/{fileName}.err", 'wb') as err:
        subprocess.run(
          ["../target/debug/cli", "-d"], 
          input=input, 
          stdout=out,
      ...
      convert_to_hex(f"{baseDir}/binary/{fileName}.bin", f"{baseDir}/hex/{fileName}.hex")
