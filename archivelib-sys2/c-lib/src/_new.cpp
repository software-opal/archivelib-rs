...
//
#elif defined(AL_MICROSOFT) && defined(AL_WINDOWS_MEMORY) &&                   \
    defined(AL_LARGE_DATA)
...
//
#elif defined(AL_WINDOWS_MEMORY) && !defined(AL_FLAT_MODEL) &&                 \
    !defined(AL_LARGE_DATA) && !defined(AL_WATCOM)
...
int PointerInHeap(void *p) {
  ...
  //
  _asm mov ax, ds _asm mov wHeap, ax LEntry.dwSize = sizeof(LOCALENTRY);
...
void operator delete(void *ptr) {
  ...
  AL_ASSERT(!IsBadWritePtr(ptr, 1),
            "delete: delete called for ptr Windows doesn't like");
#endif
  char *p = (char *)ptr;
  AL_ASSERT(PointerInHeap(p - 8),
            "delete: delete called for pointer not found in the " LIB_TYPE
            " heap");
  AL_ASSERT(
      ((long *)p)[-1] == 0x12345678L,
      "delete : Data corrupted in object's leading picket in the " LIB_TYPE
      " heap");
  ...
  AL_ASSERT(
      picket == 0xfedcba98L,
      "delete : Data corrupted in object's trailing picket in the " LIB_TYPE
      " heap");
